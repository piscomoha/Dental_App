...
class StoreRendezVousRequest extends FormRequest
{
    ...
    {
        return [
            'date_rdv' => 'required|date_format:Y-m-d|after_or_equal:today',
            'heure_rdv' => 'required|date_format:H:i',
            'statut' => 'nullable|in:En attente,Confirmé,Annulé,Terminé',
            'patient_id' => 'required|exists:patients,id|integer|min:1',
            'dentiste_id' => 'nullable|integer',
            'secretaire_id' => 'nullable|integer',
        ];
    }
